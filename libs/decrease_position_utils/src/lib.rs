...
pub fn decrease_position(env: &Env, p: &DecreasePositionParams) -> DecreasePositionResult {
    ...
    if output_amount > 0 {
        if p.swap_path.is_empty() {
            ...
            MarketTokenClient::new(env, &p.market.market_token)
                .withdraw_from_pool(p.caller, p.collateral_token, &first_market, &output_amount);
            let (_out_token, swapped) = swap_with_path(
                env, p.data_store, p.caller, p.oracle,
                p.collateral_token, output_amount,
                &p.swap_path, p.receiver,
            );
...
mod tests {
    ...
    fn configure_market(w: &World, fee_bps: i128) {
        ...
        ds_c.set_u128(&w.admin, &gmx_keys::max_leverage_key(&w.env, &w.market_tk), &(50 * FP as u128));
        ...
        ds_c.set_u128(&w.admin, &gmx_keys::pool_amount_key(&w.env, &w.market_tk, &w.short_tk),  &(10_000 * ONE_TOKEN as u128));
    }

    /// Store a synthetic position in the env's persistent storage so `decrease_position` can load it.
    fn plant_position(w: &World, size_usd: i128, collateral: i128, index_price: i128) -> PositionProps {
    ...
    fn partial_close_position_fee_matches_formula() {
        ...
        assert!(result.output_amount >= 0, "output must be non-negative, got {}", result.output_amount);
        ...
        assert!(diff <= 2,
            "output_amount={} expected={} diff={}", result.output_amount, expected_output, diff);

        // Partial close: position still open, remaining collateral is positive
        assert!(!result.is_fully_closed, "should be partial close");
        assert!(result.remaining_collateral > 0, "remaining collateral must be positive after partial close");
    }
    ...
    fn full_close_output_non_negative_and_fully_closed() {
        ...
        assert_eq!(result.remaining_collateral, 0, "no collateral remains after full close");
    }
    ...
    fn output_clamped_to_zero_on_loss_exceeding_collateral() {
        ...
        assert!(result.output_amount >= 0, "output_amount must not underflow; got {}", result.output_amount);
    }
    ...
    fn partial_close_residual_claimable_funding_is_correct() {
        ...
        let claim_key = gmx_keys::claimable_funding_amount_key(&w.env, &w.market_tk, &w.long_tk, &w.user);
        let claimable = ds_c.get_u128(&claim_key);
        assert!(claimable > 0, "claimable funding must be positive after partial close with owed funding");
    }
}
